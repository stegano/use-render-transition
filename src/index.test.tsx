...
import { useTransitionRender } from "./hooks";
import { TransitionRenderProvider } from "./providers";
import { createStore } from "./store/store";
import { defaultStore } from "./providers/transition-render";
...
describe("`useTransitionRender` Testing", () => {
  beforeEach(async () => {
    defaultStore._reset();
  });
  ...
  it("shared data", async () => {
    const TestComponentA = () => {
      ...
      const [render, handleData] = useTransitionRender<string>(undefined, "share1");
      return render(
        (data, prevData) => {
          return (
            <button
    ...
    const TestComponentB = () => {
      const [render] = useTransitionRender<string>(undefined, "share1");
      return render(
        (data, prevData) => {
          return (
            <button type="button">
  ...
  it("shared data(after the status processing is updated, create a hook)", async () => {
    const TestComponentA = () => {
      ...
      const [render, handleData] = useTransitionRender<string>(undefined, "share2");
      useEffect(() => {
        handleData(async () => {
          await task();
          return "Aaa";
    ...
    const TestComponentB = () => {
      const [render] = useTransitionRender<string>(undefined, "share2");
      return render(
        (data, prevData) => {
          return (
            <button type="button">
  ...
  it("shared error data", async () => {
    const TestComponentA = () => {
      const [render, handleData] = useTransitionRender<string>(undefined, "share error 1");
      useEffect(() => {
        handleData(async () => {
          return "Aaa";
        });
    ...
    const TestComponentB = () => {
      const [render] = useTransitionRender<string>(undefined, "share error 1");
      return render(
        (data, prevData) => {
          return (
            <button type="button">
  ...
  it("shared error data(after the status processing is updated, create a hook)", async () => {
    const TestComponentA = () => {
      const [render, handleData] = useTransitionRender<string>(undefined, "share error 2");
      useEffect(() => {
        handleData(async () => {
          return "Aaa";
        });
    ...
    const TestComponentB = () => {
      const [render] = useTransitionRender<string>(undefined, "share error 2");
  ...
  it("When the `DataResetHandler` function is invoked, assert the state", async () => {
    const TestComponent = () => {
      const [renderData, , handleDataReset] = useTransitionRender<string>({
        default: "Test",
      });
      return renderData(
        (data) => (
  ...
  it("shared data with custom store", async () => {
    ...
    const componentA = ReactTestRender.create(
      <TransitionRenderProvider store={customStore}>
        <TestComponentA />
      </TransitionRenderProvider>,
    );
    await ReactTestRender.act(async () => {
    ...
    await ReactTestRender.act(async () => {
      ...
      expect(customStore.getSnapshot()).toEqual({
        share1: { data: "Bbb", previousData: "Aaa", status: "SUCCESS" },
      });
      componentA.unmount();
      componentB.unmount();
    });
